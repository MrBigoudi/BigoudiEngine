...
void RayTracer::run(Vector3 backgroundColor, bool verbose){
    if(!_IsRunning){
        ...
        for(uint j = 0; j<_Image->getHeight(); j++){
            for(uint32_t i=0; i<_Image->getWidth(); i++){
                // fprintf(stdout, "j: %d, i:%d\n", j, i);
                Ray curRay = _Camera->rayAt(i, j);
                ...
                Vector3 color = (1.f-a)*Vector3() + a*Vector3(0.f, 1.f, 0.f);
